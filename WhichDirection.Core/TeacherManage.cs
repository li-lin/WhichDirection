...
namespace WhichDirection.Core
{
    ...
    public class TeacherManage
    {
        ...
        public Teacher GetTeacher(string msg)
        {
            ...
            }else if(dbContext.Teachers.Where(x => x.Direction == msg).FirstOrDefault() != null)
            {
                return dbContext.Teachers.Where(x => x.Direction == msg).FirstOrDefault();
            }
            else
            {
                return null;
        ...
        /// </summary>
        /// <param name="id">教师ID</param>
        /// <param name="authority">权限(管理员=1，教师=-1)</param>
        /// <returns></returns>
        public bool AddAuthority(int id,int authority)
        {
